...
#     def __init__(self):
#         self.model = None
#         self.model_path = Path(__file__).parent.parent.parent.parent / 'models' / 'mobilenet_v2_food_production.keras'
...
class MLPredictor:
    """
    Production ML Predictor aligned EXACTLY with training pipeline
    Includes Focal Loss support and optimal threshold from evaluation
    """
    ...
    def __init__(self):
        ...
        self.target_size = (224, 224)
        self.model_path = Path(__file__).parent.parent.parent.parent / "models" / "mobilenet_v2_food_production.keras"
        self.threshold = 0.5  # Default, will be updated from config
    ...
    def _load_threshold_config(self):
        ...
        threshold_file = self.model_path.parent / "threshold_config.json"
        
        if threshold_file.exists():
            try:
                with open(threshold_file, 'r') as f:
                    config = json.load(f)
                    self.threshold = config.get('OPTIMAL_THRESHOLD', 0.5)
            ...
            logger.info(f"[THRESHOLD] Config file not found. Using default 0.5")
            logger.info(f"[RECOMMENDED] Run evaluate.py to find optimal threshold")
            self.threshold = 0.5

    def _load_model(self):
        """Load model with Focal Loss support"""
        try:
            if self.model is None:
                if not self.model_path.exists():
                    raise FileNotFoundError(f"Model not found at {self.model_path}")

                # Import FocalLoss from model pipeline
                try:
                    ...
                    custom_objects = {}
                    logger.warning("Could not import FocalLoss, using default custom objects")
                finally:
    ...
    def predict_single(self, image_data: Union[bytes, np.ndarray]) -> Tuple[str, float]:
        ...
        try:
            ...
            logger.debug(f"Prediction: raw={prediction:.4f}, threshold={self.threshold:.4f}, class={predicted_class}, conf={confidence:.4f}")
...
def process_product_images(images: Dict[str, bytes], brand_name: str):
    ...
    try:
        ...
        for view_type, image_data in images.items():
            label, confidence = ml_predictor.predict_single(image_data)
        ...
        final_score = round(np.mean(confidences) * 100, 2)
